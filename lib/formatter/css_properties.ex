defmodule BuenaVista.CssProperties do
  # Raw source: https://github.com/csscomb/csscomb.js/blob/dev/config/yandex.json
  ...
  @property_index [
                  ...
                  |> then(fn lists ->
                    for {properties, list_idx} <- Enum.with_index(lists) do
                      for {property, prop_idx} <- Enum.with_index(properties) do
                        {property, list_idx * 1000 + prop_idx}
                      end
                    end
                  end)
                  |> List.flatten()
  ...
  defp selector_type("*" <> _rest), do: 4
  defp selector_type(_selector), do: 5
